...
int inc_vma_limit(struct pcb_t *caller, int vmaid, addr_t inc_sz)
{
  ...
  addr_t aligned = PAGING_ALIGN(inc_sz);
  addr_t old_end = cur_vma->sbrk;
  addr_t new_end = old_end + aligned;
  if(validate_overlap_vm_area(caller, vmaid, old_end, new_end) <0) return -1;
  int num_pasges = aligned / PAGING_PAGESZ;
  struct vm_rg_struct *rg = get_vm_area_node_at_brk(caller, vmaid, inc_sz, aligned);
  if(vm_map_ram(caller, rg->rg_start, rg->rg_end, old_end, num_pasges, rg) <0) {
      return -1;
  }
...
/*
 * Virtual Memory Manager (mm-vm.c)
 * Clean + Correct Implementation for CO2018 - LamiaAtrium
 */
